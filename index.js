...
function removeLastElement(arr) {
  ...
  // if (arr.length === 1) {
  //   return arr.pop(); // Повертає Видалений останній елемент або `undefined`, якщо масив порожній.
  // } else if (arr.length === 0) {
  //   return undefined; // Повертає Видалений останній елемент або `undefined`, якщо масив порожній.
  // } else {
  //   arr.pop();
...
// console.log(removeLastElement([9])); // Виведе 9
// console.log(removeLastElement([])); // Виведе undefined
...
function getElementAtIndex(arr, index) {
  ...
  if (arr.length != 0) {
    if (index > 0 && index < arr.length) return arr.at(index);
  }
  return undefined;
}
...
function findElementIndexes(arr, element) {
  ...
  if (Array.isArray(arr)) {
    const ind_first = arr.indexOf(element);
    const ind_last = arr.lastIndexOf(element);
    if (ind_first !== -1) index.push(ind_first);
    if (ind_last !== -1 && ind_last !== ind_first) index.push(ind_last);
  }
  return index;
}
...
// console.log(findElementIndexes([1, 2, 3, 4, 5, 3], 2)); //Виведе [ 1 ]
// console.log(findElementIndexes([1, 3, 3, 4, 5, 3], 2)); //Виведе [  ]
...
function copyAndSwapElements(arr, target, start, end) {
  ...
  else arr.copyWithin(target, start, end);
  return arr;
}
...
function customFill(arr, value, start = 0, end = arr.length) {
  ...
  if (!Array.isArray(arr) || start < 0 || start > end || end > arr.length) {
    return false;
  }
...
function customAt(arr, index) {
  ...
  if (!Array.isArray(arr) || index < 0 || index >= arr.length) {
    ...
    const getElement = arr.at(index);
    switch (typeof getElement) {
      case "string":
